...
const fallback:Vehicle = {title:"2026 Range Rover Velar P250 Dynamic SE",price:58780,mileage:7400,stock:"SR26115",vin:FEATURED_VIN,exterior:"Fuji White",interior:"Cloud",image:"/velar-hero.png",url:"https://www.landroverwillowgrove.com/llm/inventory/"};
    ...
    <div className="shell" style={{position:"relative",zIndex:1}}>
      ...
      <article style={{marginTop:"34px",border:"1px solid rgba(242,238,230,.28)",display:"grid",gridTemplateColumns:"minmax(0,1.35fr) minmax(300px,.65fr)",background:"rgba(10,15,12,.38)"}} className="featuredVehicleCard">
        ...
        <div style={{padding:"clamp(28px,4vw,44px)",display:"flex",flexDirection:"column",justifyContent:"space-between",gap:"32px",borderLeft:"1px solid rgba(242,238,230,.28)",background:"rgba(8,12,10,.68)"}}>
          <div><p className="eyebrow" style={{color:"#b3bbb7",margin:"0 0 18px"}}>LIVE WILLOW GROVE LISTING</p><div style={{fontSize:"clamp(38px,5vw,64px)",fontWeight:300,letterSpacing:".02em",marginBottom:"6px"}}>{money(vehicle.price)}</div><div style={{fontSize:"9px",letterSpacing:".14em",color:"#9fa8a3",marginBottom:"24px"}}>INCLUDES {money(DOC_FEE)} DEALER DOCUMENTATION FEE · TAX, TITLE &amp; REGISTRATION ADDITIONAL</div><div style={{display:"grid",gap:"11px",fontSize:"11px",letterSpacing:".13em",color:"#d7dbd8"}}>{miles(vehicle.mileage)&&<span>{miles(vehicle.mileage)}</span>}<span>{vehicle.exterior||""}{vehicle.exterior&&vehicle.interior?" · ":""}{vehicle.interior||""}</span>{vehicle.stock&&<span>STOCK {vehicle.stock}</span>}<span>VIN {vehicle.vin}</span></div></div>
