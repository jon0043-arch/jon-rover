...
async function fetchReader(target:string, timeoutMs=4500){
  const controller=new AbortController();
  const timer=setTimeout(()=>controller.abort(),timeoutMs);
  try{
    const r=await fetch(readerUrl(target),{cache:"no-store",headers:{Accept:"text/plain"},signal:controller.signal});
...
async function enrichVehicle(v:Vehicle):Promise<Vehicle>{try{const text=await fetchReader(v.url,3000);const image=text.match(/!\[[^\]]*\]\((https?:\/\/[^)]+\.(?:jpg|jpeg|png|webp)(?:\?[^)]*)?)\)/i)?.[1]??null;const stock=text.match(/Stock(?: Number| #|:)?\s*[:#]?\s*([A-Z0-9-]{4,})/i)?.[1]??null;const exterior=text.match(/Exterior(?: Color)?\s*[:|]\s*([^\n|]{2,80})/i)?.[1]?.trim()??null;const interior=text.match(/Interior(?: Color)?\s*[:|]\s*([^\n|]{2,80})/i)?.[1]?.trim()??text.match(/Interior\s*[:\-]\s*([^\n]{2,80})/i)?.[1]?.trim()??null;const interiorFamily=normalizeInteriorFamily(interior);const features=Array.from(new Set([...text.matchAll(/(?:Feature|Equipment|Package)\s*[:|]\s*([^\n|]{3,80})/gi)].map(m=>m[1].trim()).filter(Boolean))).slice(0,8);return{...v,image,stock,exterior,interior,interiorFamily,features};}catch{return v;}}
export async function GET(request:NextRequest){const q=request.nextUrl.searchParams.get("q")??"";const condition=(request.nextUrl.searchParams.get("condition")??"all").toLowerCase();const limit=Math.min(Math.max(Number(request.nextUrl.searchParams.get("limit")??12)||12,1),18);try{
const pageUrls=[INVENTORY_URL,`${INVENTORY_URL}?_p=2`,`${INVENTORY_URL}?_p=3`];
const texts=await Promise.all(pageUrls.map(async u=>{try{return await fetchReader(u,5000)}catch{return""}}));const parsed=texts.flatMap(parseReaderInventory);let unique=Array.from(new Map(parsed.map(v=>[v.vin,v])).values());if(!unique.length)return NextResponse.json({error:"Willow Grove inventory could not be read right now.",vehicles:[],count:0},{status:502});if(condition==="new")unique=unique.filter(v=>v.condition.toLowerCase()==="new");if(condition==="used")unique=unique.filter(v=>v.condition.toLowerCase()!=="new");
const totalMatch=texts.join("\n").match(/([\d,]+)\s+vehicles found/i)?.[1];
// No search query: return the inventory list immediately. Do NOT hold the page open while scraping detail pages.
if(!q.trim()) return NextResponse.json({total:totalMatch?Number(totalMatch.replace(/,/g,"")):unique.length,count:Math.min(unique.length,limit),query:q,condition,vehicles:unique.slice(0,limit),source:"Land Rover Willow Grove",syncMethod:"fast-list",updatedAt:new Date().toISOString()});
let candidates=unique;if(wantsSevenSeats(q))candidates=candidates.filter(thirdRowFamily);const model=requestedModel(q);if(model){const modelOnly=candidates.filter(v=>modelMatches(v,model));if(modelOnly.length)candidates=modelOnly;}
...
const enrichmentPoolSize=(color||interior)?Math.min(candidates.length,20):wantsSevenSeats(q)?Math.min(candidates.length,16):Math.min(candidates.length,8);
...
let enriched=await Promise.all(ranked.map(enrichVehicle));
...
return NextResponse.json({total:totalMatch?Number(totalMatch.replace(/,/g,"")):unique.length,count:enriched.length,query:q,condition,vehicles:enriched,source:"Land Rover Willow Grove",syncMethod:"timeout-safe-detail-search",updatedAt:new Date().toISOString()});}catch(e){console.error("Inventory route failed",e);return NextResponse.json({error:"Live inventory is temporarily unavailable."},{status:502});}}
