...
async function loadInventoryPages(){
  const firstUrl=`${INVENTORY_URL}?limit=100&page=1`;
  const first=await fetchReader(firstUrl);
  const pageMatch=first.match(/Page\s+1\s+of\s+(\d+)/i);
  const pageCount=Math.max(1,Math.min(Number(pageMatch?.[1]||1),20));
  if(pageCount===1)return [first];
  const rest=await Promise.all(Array.from({length:pageCount-1},(_,i)=>fetchReader(`${INVENTORY_URL}?limit=100&page=${i+2}`).catch(()=>"")));
  return [first,...rest];
}
...
const texts=await loadInventoryPages();const parsed=texts.flatMap(parseReaderInventory);let unique=Array.from(new Map(parsed.map(v=>[v.vin,v])).values());if(!unique.length)return NextResponse.json({error:"Willow Grove inventory could not be read right now.",vehicles:[],count:0},{status:502});if(condition==="new")unique=unique.filter(v=>v.condition.toLowerCase()==="new");if(condition==="used")unique=unique.filter(v=>v.condition.toLowerCase()!=="new");let candidates=unique;if(wantsSevenSeats(q))candidates=candidates.filter(thirdRowFamily);const model=requestedModel(q);if(model){const modelOnly=candidates.filter(v=>modelMatches(v,model));if(modelOnly.length)candidates=modelOnly;}
const budget=parseBudget(q);if(budget!=null)candidates=candidates.filter(v=>v.price!=null&&v.price<=budget+10000);
if(browse){const browsed=candidates.slice(0,limit);const totalMatch=texts.join("\n").match(/([\d,]+)\s+vehicles found/i)?.[1];return NextResponse.json({total:totalMatch?Number(totalMatch.replace(/,/g,"")):unique.length,count:browsed.length,query:q,condition,vehicles:browsed,source:"Land Rover Willow Grove",syncMethod:"reader-full-browse-real-pagination",updatedAt:new Date().toISOString()});}
