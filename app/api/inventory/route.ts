...
function mergeInventory(saved:Vehicle[],bundled:Vehicle[]){if(!saved.length)return bundled;const byVin=new Map(bundled.map(v=>[v.vin.toUpperCase(),v]));const seen=new Set(saved.map(v=>v.vin.toUpperCase()));const merged=saved.map(db=>{const snap=byVin.get(db.vin.toUpperCase());return snap?hydrateVehicle({...db,...snap,stock:validMeta(snap.stock)||validMeta(db.stock),exterior:validMeta(snap.exterior)||validMeta(db.exterior),interior:validMeta(snap.interior)||validMeta(db.interior),interiorFamily:validMeta(snap.interiorFamily)||validMeta(db.interiorFamily)}):db});for(const snap of bundled)if(!seen.has(snap.vin.toUpperCase()))merged.push(snap);return merged;}
