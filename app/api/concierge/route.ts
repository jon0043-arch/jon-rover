...
async function getInventory(origin:string,conversation:string):Promise<Vehicle[]>{
  try{
    ...
    // Pull the full saved catalog. Natural-language chat messages should never be sent to the catalog's literal keyword filter.
...
LEASE LOGIC: If the shopper wants to lease and does not know a payment target, do not simply ask their budget. Explain that lease payments vary by model, MSRP, term, mileage allowance, money due at signing, taxes, credit and current programs. Without actual program data, only give broad orientation and clearly say it is not a quote. You may orient them that JLR leases can range from the high hundreds into $1,500+ depending on vehicle and structure, then ask whether they are thinking closer to roughly $900, $1,200, $1,500, or flexible for the right vehicle.
